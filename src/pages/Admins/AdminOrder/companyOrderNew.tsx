...
import { Button, Col, Form, Input, message, Row, Space, Spin, Radio } from 'antd';
...
export default (props: any, childRef: any) => {
  ...
  const submitAdd = (value: any) => {
    ...
    params.studentUserId=value.studentUserId;
    //新增学员订单时添加学员studentid 不添加则为修改订单
    if (renderData.typee == 'add') {
  ...
  const submitEidt = (value: any) => {
    ...
    value = { ...value, ...standards[0], ...JSON.parse(standards[0].JobClassExam),studentUserId: studentUserId };
