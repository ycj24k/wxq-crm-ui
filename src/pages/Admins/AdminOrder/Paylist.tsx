...
const PayWay = forwardRef<PayWayMethods, any>((props, ref) => {
    ...
    useEffect(() => {
        if (chargeLog) {
            ...
            // 获取当前活动的表单索引（通常是第一个）
            const currentIndex = payWayList[0];
            // 确保表单引用存在
            if (formRefs.current[currentIndex]) {
                // 设置收款记录ID
                formRefs.current[currentIndex].setFieldsValue({
                ...
                // 如果有用户ID，设置用户信息
                if (chargeLog[0].userId) {
                    // 更新用户选择组件
                    if (userRefs.current[currentIndex]) {
                        userRefs.current[currentIndex].setDepartment({
