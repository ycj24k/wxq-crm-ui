...
export default (props: any) => {
  ...
  //   console.log(content,'content======>depar')

  //   setTreeData(content);
  // };

  const getMenuTree = async () => {
    const contentList: any = await request.get('/sms/system/sysMenu/tree');
    // 过滤掉已删除的节点
    const filteredData = filterDeletedNodes(contentList.data);
    // 设置过滤后的原始数据到 menuData 状态中
  ...
  return (
    <ProCard title="菜单列表" colSpan="25%" style={{ width: type == 'modal' ? '100%' : '25%' }}>
      ...
      {menuData.length > 0 && (
        <Tree
          ...
          treeData={searchValue ? loop(convertToTreeData(menuData)) : convertToTreeData(menuData)}
        />
      )}
    </ProCard>
  );
